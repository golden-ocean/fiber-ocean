...
func (dq *DictionaryQuery) QueryItems() *DictionaryItemQuery {
	query := (&DictionaryItemClient{config: dq.config}).Query()
...
func (dq *DictionaryQuery) WithItems(opts ...func(*DictionaryItemQuery)) *DictionaryQuery {
	query := (&DictionaryItemClient{config: dq.config}).Query()
...
func (dq *DictionaryQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Dictionary, error) {
	...
	if query := dq.withItems; query != nil {
		if err := dq.loadItems(ctx, query, nodes,
			func(n *Dictionary) { n.Edges.Items = []*Dictionary_Item{} },
			func(n *Dictionary, e *Dictionary_Item) { n.Edges.Items = append(n.Edges.Items, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (dq *DictionaryQuery) loadItems(ctx context.Context, query *DictionaryItemQuery, nodes []*Dictionary, init func(*Dictionary), assign func(*Dictionary, *Dictionary_Item)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Dictionary)
	for i := range nodes {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(dictionary_item.FieldDictionaryID)
	}
	query.Where(predicate.Dictionary_Item(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(dictionary.ItemsColumn), fks...))
	}))
