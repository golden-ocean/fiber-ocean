...
func (oq *OrganizationQuery) QueryStaffs() *StaffQuery {
	query := (&StaffClient{config: oq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := oq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (oq *OrganizationQuery) WithStaffs(opts ...func(*StaffQuery)) *OrganizationQuery {
	query := (&StaffClient{config: oq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (oq *OrganizationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Organization, error) {
	...
	if query := oq.withStaffs; query != nil {
		if err := oq.loadStaffs(ctx, query, nodes,
			func(n *Organization) { n.Edges.Staffs = []*Staff{} },
			func(n *Organization, e *Staff) { n.Edges.Staffs = append(n.Edges.Staffs, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (oq *OrganizationQuery) loadStaffs(ctx context.Context, query *StaffQuery, nodes []*Organization, init func(*Organization), assign func(*Organization, *Staff)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Organization)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(staff.FieldOrganizationID)
	}
	query.Where(predicate.Staff(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(organization.StaffsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
